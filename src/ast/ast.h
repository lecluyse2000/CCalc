...
class BoolAST {
   public:
    BoolAST(const std::span<Types::Token> expression) noexcept;
    [[nodiscard]] bool evaluate() const;

   private:
    std::unique_ptr<BoolNode> build_ast() noexcept;
    const std::span<Types::Token> m_prefix_expression;
    std::size_t m_index;
...
class MathAST {
   public:
    MathAST(const std::span<Types::Token> expression, const bool _floating_point);
